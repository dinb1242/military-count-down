...
export const markdownMulterOptions: MulterOptions = {
  // 파일에 대한 유효성을 검증한다.
  ...
  storage: diskStorage({
    destination: (request, file, callback) => {
      file.originalname = file.originalname.replace(' ', '').replace('\\', '_');
      const { email }: any = request.user;
      const uploadPath = `public/${email}`;
    ...
    filename: (req, file, callback) => {
      callback(null, randomUUID() + file.originalname.replace(' ', ''));
    },
  }),
};
//
